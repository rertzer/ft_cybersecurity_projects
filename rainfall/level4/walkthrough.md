...
- Same process situation as before, except that now the `printf` is in another function and the value to set is 16930116.
- And this time, it will directly print the result in the console, no shell.
...
- We use the same method as before. The address of the variable `m` is 0x8049810.
- First we found the position of the buffer (`AAAA` ASCII code is `0x41414141`) and, as before, accessing it makes the program segfault.
