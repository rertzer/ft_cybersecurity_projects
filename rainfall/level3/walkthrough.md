...
- Here we have a program reading on stdin again.
- But by decompiling the code, whe can see a `fgets()` function reading 512 bytes and storing them in a buffer of size 520.
...
- Let's introduce our new exploit the **Format string attack**.
- This attack uses this missuse of `printf` functions to interact with the memory.
- More precisely, we can use thoses three formats to play:

  - `%x` reads data from the stack;
  - `%s` reads character strings from the memory;
  - `%n` writes an integer to locations in the memory.
...
- Here, `printf` returns the first value in the memory so it's working!
...
- We can simplify this command bu using Direct Parameter Access.
...
- Here we can see our buffer in hexadecimal, meaning we found the emplacement.
...
- In this example, we are printing 0x41414141 who segfault because it does not exist.
- But we can try with something existing like the argument of the system function `/bin/sh` (0x804860d).
...
- Look it's our memory!! Now, let's try now to modify `m` with this.
- For this we need the address of "m":
...
- As we saw, `m` is now equal to 64 and we run the shell.
