...
- On a 32-bit architecture, the canary is 4 bytes long. 3 bytes hold a random number provided by the `execve()` system call (a 16 bytes random number is put above the argument and environment strings), and the least significant byte is replaced by 0.
